...
# ── MELHORIA 1: K dinâmico — jogos da temporada corrente têm mais peso no ELO ──
# Quanto mais recente o jogo dentro do 2026, maior o K
K_HISTORICAL = 32   # jogos de temporadas anteriores
K_CURRENT    = 48   # jogos da temporada atual (2026)
K_RECENT     = 56   # últimas 5 rodadas do campeonato atual

# ── MELHORIA 2: Times que disputam Libertadores 2026 ──
# Penalidade de performance aplicada em jogos domésticos
# Ajuste conforme necessário a cada temporada
LIBERTADORES_TIMES = {
    "Flamengo", "CR Flamengo", "Fluminense", "Fluminense FC",
    "Atletico Mineiro", "CA Mineiro", "São Paulo FC",
}
LIBERTADORES_PENALTY = 0.92   # 8% de redução na probabilidade de vitória

# ── MELHORIA 3: SEASON_WEIGHTS atualizados para dar mais peso à temporada corrente ──
SEASON_WEIGHTS_ELO = {
    2026: 2.0,   # ← novo: temporada atual vale o dobro
    2025: 1.5,
    2024: 1.0,
    2023: 0.7,
    2022: 0.5,
...
def update_elo(ra, rb, result, k=32):
    """Atualiza ELO com K configurável."""
    ea = expected_score(ra, rb)
    sa, sb = (1, 0) if result == "H" else ((0, 1) if result == "A" else (0.5, 0.5))
    return ra + k * (sa - ea), rb + k * (sb - ea)

def update_elo_dynamic(ra, rb, result, season, matchday=None, max_matchday=38):
    """
    MELHORIA 1: ELO com K dinâmico.
    - Temporada atual (2026): K maior (48)
    - Últimas 5 rodadas da temporada atual: K ainda maior (56)
    - Histórico: K padrão (32)
    """
    if season == SEASON:
        if matchday is not None and matchday >= (max_matchday - 5):
            k = K_RECENT
        else:
            k = K_CURRENT
    else:
        k = K_HISTORICAL
    return update_elo(ra, rb, result, k=k)
...
def calc_position_trend(team, live_table_history, n_rounds=5):
    """
    MELHORIA 4: Tendência de posição nas últimas N rodadas.
    Retorna valor negativo se o time está subindo (bom sinal).
    Ex: -3 significa que subiu 3 posições nas últimas 5 rodadas.
    """
    if len(live_table_history) < 2:
        return 0.0
    recent = live_table_history[-n_rounds:]
    if not recent:
        return 0.0
    pos_first = recent[0].get(team, {}).get("pos", 10)
    pos_last  = recent[-1].get(team, {}).get("pos", 10)
    return float(pos_last - pos_first)   # negativo = subiu = bom


def compute_features(home, away, live_table, elo_ratings, history,
                     mv_dict, odds_dict, all_teams, max_val,
                     live_table_history=None):
    ...
    # ── MELHORIA 3: feature de Libertadores (fadiga/foco dividido) ────────────
    home_liberta = 1 if home in LIBERTADORES_TIMES else 0
    away_liberta = 1 if away in LIBERTADORES_TIMES else 0

    # ── MELHORIA 4: tendência de posição ──────────────────────────────────────
    h_pos_trend = 0.0
    a_pos_trend = 0.0
    if live_table_history:
        h_pos_trend = calc_position_trend(home, live_table_history)
        a_pos_trend = calc_position_trend(away, live_table_history)
    ...
    return {
        ...
        "home_pos_trend": h_pos_trend,
        "away_pos_trend": a_pos_trend,
        "pos_trend_diff": h_pos_trend - a_pos_trend,
        # MELHORIA 5: equilíbrio — ajuda na detecção de empates
        "aprov_equilibrio":   1.0 / (1 + abs(h_aprov - a_aprov)),
        "h2h_draw_dominance": h2h_d / max(h2h_hw + h2h_aw + h2h_d, 1),
    }
...
def run_simulation(sim_id, fixtures_by_round, completed, teams,
    ...
    # MELHORIA 4: histórico de snapshots da tabela por rodada (para tendência)
    live_table_history = []
    current_round_idx  = 0

    def snapshot_table():
        """Salva snapshot da posição atual de cada time."""
        sorted_t = sorted(live_table.items(),
                          key=lambda x: (-x[1]["pts"], -x[1]["gd"], -x[1]["gf"]))
        return {t: {"pos": i + 1} for i, (t, _) in enumerate(sorted_t)}

    def apply_result(h, a, hg, ag, season=SEASON, matchday=None):
        ...
        # MELHORIA 1: ELO com K dinâmico
        elo_ratings[h], elo_ratings[a] = update_elo_dynamic(
            elo_ratings.get(h, 1500), elo_ratings.get(a, 1500),
            res, season=season, matchday=matchday
        )
        history.append({"home": h, "away": a, "hg": hg, "ag": ag})

    # Jogos já disputados (com matchday real se disponível)
    for item in completed:
        if len(item) == 5:
            h, a, hg, ag, md = item
        else:
            h, a, hg, ag = item; md = None
        apply_result(h, a, hg, ag, season=SEASON, matchday=md)
        current_round_idx += 1

    # Snapshot após jogos já disputados
    if live_table:
        live_table_history.append(snapshot_table())

    # Simular por rodada — batch predict por rodada
    for rd_idx, round_games in enumerate(fixtures_by_round):
        if not round_games:
            continue

        # Número aproximado da rodada futura
        current_matchday = n_completed_rounds + rd_idx + 1
        ...
        for h, a in round_games:
            f = compute_features(h, a, live_table, elo_ratings, history,
                                 ...
                                 live_table_history=live_table_history)
        ...
        # MELHORIA 3: Penalidade Libertadores — reduz P(vitória) do time afetado
        for i, (h, a) in enumerate(round_games):
            if h in LIBERTADORES_TIMES:
                ph_arr[i] *= LIBERTADORES_PENALTY
            if a in LIBERTADORES_TIMES:
                pa_arr[i] *= LIBERTADORES_PENALTY
        ...
        # Aplicar resultados
        for i, (h, a) in enumerate(round_games):
            # MELHORIA 5: threshold adaptativo — mesma lógica do match_model
            # O sorteio usa as probabilidades calibradas; em seguida ajusta
            # o placar para ser consistente com o resultado sorteado.
            ph_i, pd_i, pa_i = ph_arr[i], pd_arr[i], pa_arr[i]
            max_other = max(ph_i, pa_i)
            if pd_i >= draw_threshold and pd_i >= max_other * 0.85:
                res = "D"
            else:
                res = np.random.choice(["H", "A"], p=[ph_i / (ph_i + pa_i),
                                                       pa_i / (ph_i + pa_i)])
            ...
            apply_result(h, a, hg, ag, season=SEASON, matchday=current_matchday)

        # Snapshot após cada rodada simulada (para tendência)
        live_table_history.append(snapshot_table())
...
def main():
    ...
    # MELHORIA 1: pré-processar completed com matchday para ELO dinâmico
    has_matchday = "matchday" in completed_df.columns
    completed_list = []
    for _, r in completed_df.iterrows():
        md = int(r["matchday"]) if has_matchday and pd.notna(r.get("matchday")) else None
        completed_list.append(
            (r["home_team"], r["away_team"], int(r["home_goals"]), int(r["away_goals"]), md)
        )

    # Número de rodadas já disputadas (para calcular matchday futuro)
    n_completed_rounds = 0
    if has_matchday:
        n_completed_rounds = int(completed_df["matchday"].max()) if len(completed_df) > 0 else 0
    ...
    print(f"   Rodadas disputadas: {n_completed_rounds}")

    # MELHORIA 2: histórico com peso maior para temporada atual
    # Pegamos mais histórico antigo + todo o 2026 disputado
    hist_pre = hist_df[hist_df["season"] < SEASON].tail(5000)
    hist_cur = hist_df[hist_df["season"] == SEASON]  # 2026 já jogados
    hist_combined = pd.concat([hist_pre, hist_cur]).sort_values("date")

    history_init = [
        {"home": r["home_team"], "away": r["away_team"],
         "hg": int(r["home_goals"]), "ag": int(r["away_goals"])}
        for _, r in hist_combined.iterrows()
    ]
    print(f"   Histórico: {len(hist_pre)} anteriores + {len(hist_cur)} de 2026 = {len(history_init)} total")

    # MELHORIA 1: ELO inicial com K dinâmico por temporada
    elo_ratings = {t: 1500 for t in teams}
    for _, rec_row in hist_combined.iterrows():
        h, a = rec_row["home_team"], rec_row["away_team"]
        if h not in elo_ratings: elo_ratings[h] = 1500
        if a not in elo_ratings: elo_ratings[a] = 1500
        hg, ag = int(rec_row["home_goals"]), int(rec_row["away_goals"])
        res = "H" if hg > ag else ("A" if ag > hg else "D")
        md  = int(rec_row["matchday"]) if "matchday" in rec_row and pd.notna(rec_row.get("matchday")) else None
        elo_ratings[h], elo_ratings[a] = update_elo_dynamic(
            elo_ratings[h], elo_ratings[a], res,
            season=int(rec_row["season"]), matchday=md
        )

    print("\n📊 ELO inicial dos times:")
    for t, elo in sorted(elo_ratings.items(), key=lambda x: -x[1]):
        print(f"   {t:<30} {elo:.0f}")
    ...
    try:
        ...
        print(f"\n   Valores de mercado: {len(mv_dict)} times")
    except Exception:
        mv_dict = {}
    ...
    # MELHORIA 5: threshold de empate salvo pelo match_model
    draw_threshold = model_data.get("draw_threshold", 0.28)
    print(f"   ✅ Modelos carregados | draw_threshold={draw_threshold}")
    ...
    for i in range(10):
        run_simulation(i, fixtures_by_round, completed_list, teams,
                       ...
                       model_data, poisson_data, max_val,
                       feat_order, poisson_feat_h, poisson_feat_a,
                       n_completed_rounds=n_completed_rounds,
